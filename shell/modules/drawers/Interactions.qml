...
CustomMouseArea {
    ...
    function withinPanelHeight(panel: Item, x: real, y: real): bool {
        // For top bar: panels start below bar, so add bar.implicitHeight to panel.y
        const panelY = bar.implicitHeight + Config.border.thickness + panel.y;
    ...
    function inLeftPanel(panel: Item, x: real, y: real): bool {
        // For top bar: left panels are at left edge of screen (no bar offset for X)
        return x < Config.border.thickness + panel.x + panel.width && withinPanelHeight(panel, x, y);
    }

    function inRightPanel(panel: Item, x: real, y: real): bool {
        // For top bar: right panels are at right edge of screen (no bar offset for X)
        return x > Config.border.thickness + panel.x && withinPanelHeight(panel, x, y);
    }

    function inTopPanel(panel: Item, x: real, y: real): bool {
        // panel.y is now -bar.implicitHeight relative to the Panels container,
        // so bar.implicitHeight + panel.y effectively becomes 0 (screen top).
        const screenY = bar.implicitHeight + panel.y;
        return y >= screenY && y <= screenY + panel.height + Config.border.rounding && withinPanelWidth(panel, x, y);
    }
