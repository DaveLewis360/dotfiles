...
Item {
    ...
    readonly property string windowTitle: {
        const title = Hypr.activeToplevel ? Hypr.activeToplevel.title : null;
        if (!title)
            return qsTr("Desktop");
    ...
    readonly property int maxHeight: {
        ...
        const otherHeight = otherModules.reduce((acc, curr) => {
            const h = curr && curr.item ? (curr.item.nonAnimHeight ?? curr.height) : (curr ? curr.height : 0);
            return acc + h;
        }, 0);
        // Length - 2 cause repeater counts as a child
        return bar.height - otherHeight - bar.spacing * (Math.max(0, bar.children.length - 1)) - bar.vPadding * 2;
    }
