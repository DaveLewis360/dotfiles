...
Item {
    ...
    component StorageGaugeCard: StyledRect {
        ...
        readonly property var currentDisk: ((SystemUsage.disks && SystemUsage.disks.length) || 0) > 0 ? SystemUsage.disks[currentDiskIndex] : null
        ...
        clip: true
        Component.onCompleted: {
            diskCount = ((SystemUsage.disks && SystemUsage.disks.length) || 0);
            if (currentDisk)
        ...
        Connections {
            function onDisksChanged() {
                if (((SystemUsage.disks && SystemUsage.disks.length) || 0) !== storageGaugeCard.diskCount)
                    storageGaugeCard.diskCount = ((SystemUsage.disks && SystemUsage.disks.length) || 0);
